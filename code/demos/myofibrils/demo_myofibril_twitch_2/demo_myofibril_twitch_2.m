function demo_myofibril_twitch_2
...
doc_image_folder = ...
    '../../../../docs/pages/demos/myofibrils/myofibril_twitch_2'
...
exportgraphics(gcf, fullfile(doc_image_folder, 'myofibril_twitch_2.png'));
...
image_file_strings = findfiles('png',fullfile(base_dir,'sim_output'));
image_file_strings = sort_nat(image_file_strings);

animation_gif = fullfile(doc_image_folder, 'myofibril_twitch_2.gif');
for i = 1 : numel(image_file_strings)
    im = im2uint8(imread(image_file_strings{i}),256);
    [a,map] = rgb2ind(im, 256);
    if (i==1)
        imwrite(a,map,animation_gif,'gif','LoopCount',Inf,'DelayTime',0.1);
    else
        imwrite(a,map,animation_gif,'gif','WriteMode','append','DelayTime',0.1);
    end
end
